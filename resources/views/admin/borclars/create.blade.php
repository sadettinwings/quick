...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.borclar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.borclars.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cari_sec') ? 'is-invalid' : '' }}" name="cari_sec_id" id="cari_sec_id">
                    @foreach($cari_secs as $id => $entry)
                        <option value="{{ $id }}" {{ old('cari_sec_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('evsahibi_sec') ? 'is-invalid' : '' }}" name="evsahibi_sec_id" id="evsahibi_sec_id">
                    @foreach($evsahibi_secs as $id => $entry)
                        <option value="{{ $id }}" {{ old('evsahibi_sec_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('borc_aciklama') ? 'is-invalid' : '' }}" type="text" name="borc_aciklama" id="borc_aciklama" value="{{ old('borc_aciklama', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('odeme_tarihi') ? 'is-invalid' : '' }}" type="text" name="odeme_tarihi" id="odeme_tarihi" value="{{ old('odeme_tarihi') }}">
