...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.digerTahsilat.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.diger-tahsilats.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tkategori_sec') ? 'is-invalid' : '' }}" name="tkategori_sec_id" id="tkategori_sec_id">
                    @foreach($tkategori_secs as $id => $entry)
                        <option value="{{ $id }}" {{ old('tkategori_sec_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
