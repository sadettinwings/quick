...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.harcamalar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.harcamalars.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('harcama_kategori') ? 'is-invalid' : '' }}" name="harcama_kategori_id" id="harcama_kategori_id">
                    @foreach($harcama_kategoris as $id => $entry)
                        <option value="{{ $id }}" {{ old('harcama_kategori_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
