...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.personelOdemeleri.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.personel-odemeleris.update", [$personelOdemeleri->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('personel_sec') ? 'is-invalid' : '' }}" name="personel_sec_id" id="personel_sec_id">
                    @foreach($personel_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('personel_sec_id') ? old('personel_sec_id') : $personelOdemeleri->personel_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('personel_kategori') ? 'is-invalid' : '' }}" name="personel_kategori_id" id="personel_kategori_id">
                    @foreach($personel_kategoris as $id => $entry)
                        <option value="{{ $id }}" {{ (old('personel_kategori_id') ? old('personel_kategori_id') : $personelOdemeleri->personel_kategori->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tarih') ? 'is-invalid' : '' }}" type="text" name="tarih" id="tarih" value="{{ old('tarih', $personelOdemeleri->tarih) }}">
                @if($errors->has('tarih'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tarih') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tutar') ? 'is-invalid' : '' }}" type="number" name="tutar" id="tutar" value="{{ old('tutar', $personelOdemeleri->tutar) }}" step="1">
                @if($errors->has('tutar'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tutar') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('birim_sec') ? 'is-invalid' : '' }}" name="birim_sec_id" id="birim_sec_id">
                    @foreach($birim_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('birim_sec_id') ? old('birim_sec_id') : $personelOdemeleri->birim_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('aciklama') ? 'is-invalid' : '' }}" name="aciklama" id="aciklama">{{ old('aciklama', $personelOdemeleri->aciklama) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kasa_sec') ? 'is-invalid' : '' }}" name="kasa_sec_id" id="kasa_sec_id">
                    @foreach($kasa_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kasa_sec_id') ? old('kasa_sec_id') : $personelOdemeleri->kasa_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
