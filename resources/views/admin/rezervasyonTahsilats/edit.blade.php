...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.rezervasyonTahsilat.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.rezervasyon-tahsilats.update", [$rezervasyonTahsilat->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('musteri_sec') ? 'is-invalid' : '' }}" name="musteri_sec_id" id="musteri_sec_id">
                    @foreach($musteri_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('musteri_sec_id') ? old('musteri_sec_id') : $rezervasyonTahsilat->musteri_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rezervasyon_sec') ? 'is-invalid' : '' }}" name="rezervasyon_sec_id" id="rezervasyon_sec_id">
                    @foreach($rezervasyon_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('rezervasyon_sec_id') ? old('rezervasyon_sec_id') : $rezervasyonTahsilat->rezervasyon_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kasa_sec') ? 'is-invalid' : '' }}" name="kasa_sec_id" id="kasa_sec_id">
                    @foreach($kasa_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kasa_sec_id') ? old('kasa_sec_id') : $rezervasyonTahsilat->kasa_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tutar') ? 'is-invalid' : '' }}" type="number" name="tutar" id="tutar" value="{{ old('tutar', $rezervasyonTahsilat->tutar) }}" step="1">
                @if($errors->has('tutar'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tutar') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('aciklama') ? 'is-invalid' : '' }}" name="aciklama" id="aciklama">{{ old('aciklama', $rezervasyonTahsilat->aciklama) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('birim_sec') ? 'is-invalid' : '' }}" name="birim_sec_id" id="birim_sec_id">
                    @foreach($birim_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('birim_sec_id') ? old('birim_sec_id') : $rezervasyonTahsilat->birim_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
